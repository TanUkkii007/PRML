# https://github.com/ctgk/PRML/blob/master/prml/features/sigmoidal.py
...
class SigmoidalFeatures(object):
    ...
    def _sigmoid(self, x, mean):
        # tanh(x) = 2\sigma(2x) - 1
        # \sigma(x) = 0.5tanh(0.5x) + 0.5
        return np.tanh((x - mean) @ self.coef * 0.5) * 0.5 + 0.5 # '@' is matmul operator
