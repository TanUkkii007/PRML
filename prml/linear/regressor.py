# https://github.com/ctgk/PRML/blob/master/prml/linear/regressor.py
...
class Regressor(object):
    ...
    def _check_input(self, X):
        ...
        if hasattr(self, "n_features") and self.n_features != np.size(X, 1):
            raise ValueError("mismatch in dimension 1 of X(input) "
                             "(size {} is different from {}".format(
                                 np.size(X, 1), self.n_features))
